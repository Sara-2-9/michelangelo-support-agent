...
- [x] **Phase 9** — Per-conversation actions from a hover-only ellipsis menu on each sidebar row: **Share** creates/copies a public read-only link (`share_token` on the conversation, unguessable UUID; public `GET /api/share/:token` before the auth gate, RLS untouched; read-only `/share/:token` route) and **Delete** removes one conversation after a confirmation dialog (`DELETE /api/conversations/:id`, JWT + ownership checks, messages cascade)
