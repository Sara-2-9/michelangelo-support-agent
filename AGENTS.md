...
**Next step**: the project is feature-complete with CI/CD LIVE: every PR runs the full E2E gate, every merge to `main` deploys automatically with post-deploy checks. Optional follow-ups discussed: code-splitting the JS bundle (>500 kB warning), retention cleanup for old anonymous conversations (Cron Trigger job), local-only Google OAuth E2E (semi-automated storageState), eval harness in CI (manual `workflow_dispatch` or path-filtered on prompt/retrieval changes).

**Phase 11 (E2E tests + CI/CD, LIVE)**: Playwright suites in `e2e/` against the real stack (Vite dev + Worker in workerd). Two modes: `npm run test:e2e` (default env, 20 tests: smoke/cookie/mobile/API-security — write nothing) and `npm run test:e2e:staging` (STAGING Supabase project, 32 tests: adds real chat with LLM, conversation share/delete, self-service account deletion). The same suite can target any URL via `E2E_BASE_URL` (used to verify the live site post-deploy: only read-only suites — write suites self-skip without E2E_ENV=staging). **GitHub Actions**: `ci.yml` is the PR gate (typecheck + build + full staging suite; report artifact on failure), `deploy.yml` on merge to `main` (pre-deploy smoke → `wrangler deploy` with `CLOUDFLARE_DEPLOY_TOKEN` → live curl checks with edge-propagation retries). Branch protection on `main` requires the `test` check. Secrets live in GitHub repo secrets (staging keys, CF tokens, prod publishable key); the production service role key NEVER enters CI. First-CI-run learning: `auth.admin.listUsers` paginates at 50/page — seed/cleanup must paginate (staging accumulates anonymous users from every run).
