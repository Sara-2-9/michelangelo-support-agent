...
**Deploy learnings (Phase 5.4)**: (1) The Workers-AI-only API token CANNOT deploy — use `wrangler login` OAuth (scopes include workers:write). (2) Wrangler AUTO-LOADS the project `.env`, so `CLOUDFLARE_API_TOKEN` there shadows the OAuth login — `env -u` is NOT enough, the file itself is read. `npm run deploy` handles it: after the build it does `cd /tmp` and deploys with the absolute config path via `$OLDPWD`, so the `.env` is never loaded and OAuth applies. (3) `vite build` emits a self-contained `web/dist/michelangelo_support_agent/wrangler.json` — `npm run deploy` = build + deploy that config. (4) Transient "error code: 1042" right after first deploy (edge propagation); top-level try/catch in the Worker now converts any crash into a JSON 500 + console.error visible via `wrangler tail`. (5) **SPA deep links need explicit delegation**: with a Worker script present, any path that does not match a static asset reaches the WORKER — `not_found_handling = "single-page-application"` does NOT serve index.html by itself. Non-/api paths must be handed back via `env.ASSETS.fetch(request)` (requires `binding = "ASSETS"` under `[assets]`), or deep links like /privacy get the API 401. Surfaced only in Phase 8 because all earlier navigation was client-side from "/".
...
**Phase 9 (per-conversation actions)**: sidebar rows have an ellipsis button (always visible on touch breakpoints via `max-md:opacity-100`, hover/focus-only from md up) opening a small menu with **Share** (creates/reuses a random `share_token` on the conversation — new migration `20260813131300_conversation_sharing.sql` — and copies the public link to the clipboard) and **Delete** (ConfirmDialog, then `DELETE /api/conversations/:id`, messages cascade via FK; the open chat resets if it was the deleted one). Worker routes: `DELETE /api/conversations/:id` and `POST /api/conversations/:id/share` (both JWT + 404/403 ownership pattern), plus PUBLIC `GET /api/share/:token` placed BEFORE the auth gate (the unguessable token is the capability; RLS untouched — reads go through the service key). Read-only SPA route `/share/:token` (`web/src/pages/shared-conversation-page.tsx`) reuses `MessageBubble` — shared messages carry no messageId, so feedback buttons never render. Sharing is LIVE (the shared view reads current messages) and is revoked by deleting the conversation. The menu is fixed-positioned from the ellipsis rect (the list's overflow-y-auto would clip an absolute one) and closes on outside click / Escape / list scroll. Reusable `ui/confirm-dialog.tsx` extracted from the Delete Account dialog pattern.

**Phase 10 (no anonymous history, LIVE 2026-08-19 — merged via PR with merge commit)**: anonymous visitors can chat but get NO history surface — the Worker still persists their exchanges server-side (tagged with the anonymous user_id, for quality/analytics), but the client never lists their conversations, never restores/stores a `conversationId` in localStorage (reload = fresh chat; the in-memory id still threads the LIVE session for memory context). Sign-in is always a PLAIN login into a separate user id: `linkIdentity` and the `updateUser({email})` claim are GONE (together with the `?error_code=` bootstrap fallback), so the Google account chooser appears AT MOST ONCE — the "double selection" was caused by the failed link attempt bouncing the user back to Google. Anonymous conversations are never merged into a future account BY CONSTRUCTION (no code path moves rows between user ids). Sidebar for anonymous users shows a sign-in invitation card (benefits list) instead of the conversation list. Privacy policy updated: anonymous conversations are stored pseudonymously for service improvement, never shown back, never linked to later accounts. Deferred: retention/cleanup of old anonymous conversations (Cron Trigger job). The Phase 7 `linkIdentity` mechanics below are SUPERSEDED (kept as history); "Allow manual linking" in Supabase is no longer needed (inert). Entrance animation: Tailwind v4 `--animate-enter` token (fade + 14px rise, 0.55s, staggered header→main→footer via `[animation-delay:…]`), fill-mode **backwards — never `both`**: a lingering `transform` on an ancestor becomes the containing block for `position:fixed` descendants (the account panel lives inside the animated header and rendered anchored to it instead of the viewport); `backwards` hides sections during the delay but releases every property when the animation ends. `prefers-reduced-motion` disables it.

**Next step**: GitHub Actions CI/CD (planned, not started): `ci.yml` gate on PRs (typecheck + build + full Playwright staging suite) and `deploy.yml` on merge to main (smoke re-run → `wrangler deploy` → post-deploy health check), with branch protection requiring green checks. Needs GitHub secrets (staging Supabase keys, CLOUDFLARE_ACCOUNT_ID + a token with Workers Scripts:Edit — the Workers-AI-only token cannot deploy) and requires CI to create web/.env + .dev.vars.staging equivalents from secrets. Other optional follow-ups: code-splitting the JS bundle (>500 kB warning), retention cleanup for anonymous conversations, local-only Google OAuth E2E (semi-automated storageState).

**Phase 11 (E2E tests, Playwright — local, not yet in CI)**: Playwright suites in `e2e/` against the real stack (Vite dev + Worker in workerd). Two modes: `npm run test:e2e` (default env, 20 tests: smoke/cookie/mobile/API-security — write nothing) and `npm run test:e2e:staging` (STAGING Supabase project, 32 tests: adds real chat with LLM, conversation share/delete, self-service account deletion). Staging wiring: `[env.staging]` in wrangler.toml + `web/.env.staging` (browser) + `.dev.vars.staging` (Worker) — templates committed as `web/env.staging.example` / `.dev.vars.staging.example`, real files gitignored. Test user: `npm run seed:test-user` (Admin API `createUser` + `email_confirm`, no inbox). Sessions are created ONCE per run via setup projects and saved to `e2e/.auth/` (anon + seeded user) — per-test anonymous signups burst Supabase Auth's hourly rate limit (hit for real). Staging knowledge base seeded with `ENV_FILE=.dev.vars.staging npm run embed` (embed.ts honors ENV_FILE). **E2E learnings**: (1) `@cloudflare/vite-plugin` picks the worker env from `CLOUDFLARE_ENV`, NOT from Vite `--mode` — without it the worker silently uses production `.dev.vars` and staging-issued JWTs get 401. (2) Playwright `getByRole(name)` is SUBSTRING case-insensitive — "Helpful" matches "Not helpful"; use `exact: true`. (3) Two real app bugs found by the tests: the legal footer's `peer-focus-within:hidden` never worked (Tailwind `peer` needs SIBLINGS — the marker was nested inside `<main>`), and typing before the anonymous bootstrap lost the draft/mess swallowed the send (ChatProvider remounts on first session) — fixed: composer textarea+button disabled until `canSend`. (4) The conversation ellipsis menu closes on list scroll (by design): pre-scroll the row into view before opening it. (5) Magic-link testing needs no inbox: `auth.admin.generateLink`. Google OAuth is NOT automatable in CI (CAPTCHA/datacenter IPs) — only the click→redirect is tested.
...
**Docs-sync incident (2026-09-23) — learnings**: (1) The docs added a **v1 (legacy) section** next to v2 (43→57 pages); the production chunks stayed stale for weeks because the **cron trigger was not registered** on the deployed Worker — fixed by re-running `npm run deploy` (look for the `Deployed ... triggers — schedule: 37 5 * * *` line in the output; its absence means no cron). Canary for "did the sync run": `max(chunks.updated_at)` on Supabase. (2) **Manual re-index sequence is `npm run fetch:corpus` → `npm run chunk` → `npm run embed`** — running `embed` alone reads the STALE `corpus/chunks.json` and falsely reports "already up to date". (3) `wrangler login` fails with `OAuth error: request_forbidden / No CSRF value` when the DEFAULT browser has no dash.cloudflare.com session — fix: log into the dashboard in the default browser first (or paste the printed OAuth URL into the logged-in browser), then retry. (4) The dashboard "update your wrangler config" box for `observability` is informational; it does NOT imply drift — check `wrangler.toml` before touching anything.
...
web/              React SPA (Phase 5.2, restyled in Phase 6) — Vite root, own tsconfig, Tailwind CSS v4
  ...
  src/
    ...
    context/auth.tsx   AuthProvider + useAuth() — anonymous session (security plumbing only), plain OTP magic-link login, plain Google OAuth (single selection, no linking/claiming), deleteAccount (Worker → admin.deleteUser → fresh anonymous session)
...
wrangler.toml     Worker config; [assets] not_found_handling = "single-page-application";
                  [env.staging] = local-only env for E2E (never deployed)
playwright.config.ts  E2E: desktop-chromium + mobile-webkit (+ anon-setup session bootstrap;
                  + authed-chromium in staging mode)
e2e/              Playwright suites: smoke, cookie-banner, mobile, api (security), chat,
                  conversations, authed, account + setup projects (anon/user sessions)
                  and env.ts (staging env loaders)
```
...
npm run test:e2e                  # Playwright E2E, default env (20 tests, writes nothing)
npm run test:e2e:staging          # full E2E vs STAGING Supabase (32 tests, real LLM + writes)
npm run seed:test-user            # create/sync the E2E test user on staging
